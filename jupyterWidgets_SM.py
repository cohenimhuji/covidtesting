...
rel_ρ = widgets.BoundedFloatText(
    ...
    step=0.001,
    disabled=False,
    description = r'Relative Infectiousness Rate ($\rho^A/\rho^S$)',
    style = style_med,
    layout = layout_med
)
...
ωR_param = widgets.BoundedIntText(
    value=14,         # Average days with infection
    min = 1,
    max = 60,
    step= 1,
    disabled=False,
...
R_0 = widgets.BoundedFloatText(
    ...
    step=0.001,
    disabled=False,
    description = r'Base Transmission Rate ($R_0$)',
    style = style_med,
    layout = layout_med
)
...
rel_λ = widgets.BoundedFloatText(
    ...
    max = .99,
    disabled=False,
    description = r'Effectiveness of quarantine ($\lambda/\lambda^Q$)',
    style = style_med,
    layout = layout_med
)
...
Δ = widgets.BoundedFloatText(
    ...
    max = .99,
    disabled=False,
    description = r'Quarantine Scaling ($\Delta$)',
    style = style_med,
    layout = layout_med
)

slide_var = widgets.Dropdown(
    options = [(r'Quarantine Scaling (Δ)',2), (r'Testing Rate (τ)',1)],
    value = 1,
...
Timing_box = VBox([widgets.Label(value="Timing Parameters"), d_vaccine, \
                       δ_param, ωR_param], layout = box_layout_wide)
Spread_box = VBox([widgets.Label(value="2. Contagion Parameters"), R_0, rel_ρ], \
layout = box_layout_wide)
Disease_box = VBox([widgets.Label(value="3. Disease Parameters"), initial_infect, π_D], \
layout = box_layout_wide)
slide_var_box = VBox([widgets.Label(value="4. Plotting Parameters"), slide_var, slide_varOut], \
layout = box_layout_wide)
